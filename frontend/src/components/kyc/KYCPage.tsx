...
const KYCPage: React.FC = () => {
  ...
  const loadKYCDocuments = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/kyc/profile', {
        headers: {
          'Authorization': token ? `Bearer ${token}` : ''
        }
      });
      ...
      const docs: KYCDocument[] = (result.data.documents || []).map((doc: any) => ({
        ...
        fileUrl: `http://localhost:5000/api/kyc/documents/${doc._id}/download`,
            ...
            onClick={async () => {
              ...
              try {
                ...
                const response = await fetch(
                  `http://localhost:5000/api/kyc/documents/${editDialog.document?.id}`,
                  {
                    method: 'PUT',
                    headers: {
                      'Authorization': token ? `Bearer ${token}` : ''
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                const token = localStorage.getItem('token');
                const response = await fetch(
                  `http://localhost:5000/api/kyc/documents/${deleteDialog.document?.id}`,
                  {
                    method: 'DELETE',
                    headers: {
                      'Authorization': token ? `Bearer ${token}` : ''
