...
const CreatorKYCRequests: React.FC = () => {
    ...
    axios.get(url, {
      ...
      .then(res => {
        ...
        const data = (res.data.data || []).map((doc: any) => ({
          ...
          creatorId: doc.userId && doc.userId.creatorId ? doc.userId.creatorId : '-',
          name: doc.userId && doc.userId.name ? doc.userId.name : '-',
  ...
  let filtered = requests.filter(req =>
    (req.userId.toLowerCase().includes(search.toLowerCase()) ||
      ...
      req.documentNumber.toLowerCase().includes(search.toLowerCase())) &&
    (
      !statusFilter ||
