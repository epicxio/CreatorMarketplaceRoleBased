...
exports.getProfile = async (req, res) => {
  try {
    ...
    // Use assignedScreens from DB if present and non-empty
    let assignedScreens = (user.assignedScreens && user.assignedScreens.length > 0)
      ? user.assignedScreens
      : [];
    if (assignedScreens.length === 0 && user.role && user.role.permissions) {
