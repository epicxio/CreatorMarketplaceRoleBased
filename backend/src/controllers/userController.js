...
const getUsers = async (req, res) => {
  try {
    ...
    const usersWithScreens = users.map(user => {
      ...
      userObj.assignedScreens = (user.assignedScreens && user.assignedScreens.length > 0)
...
const updateUser = async (req, res) => {
  try {
    ...
    // If assignedScreens is provided in the request, save it directly
    let assignedScreensToSave = req.body.assignedScreens;
    if (!assignedScreensToSave) {
      // fallback to roleScreenMapping
      const updatedUserRole = req.body.role;
      let roleName = null;
      if (updatedUserRole && typeof updatedUserRole === 'object' && updatedUserRole.name) {
        roleName = updatedUserRole.name;
      } else if (updatedUserRole && typeof updatedUserRole === 'string') {
        // If role is an ObjectId, try to get the role name from DB
        const roleDoc = await Role.findById(updatedUserRole);
        if (roleDoc) roleName = roleDoc.name;
      }
      if (!roleName && req.body.role === undefined) {
        // fallback to existing user role
        const userDoc = await User.findById(req.params.id).populate('role');
        if (userDoc && userDoc.role && userDoc.role.name) {
      ...
      assignedScreensToSave = roleScreenMapping[roleName] || ['Dashboard'];
    }
    req.body.assignedScreens = assignedScreensToSave;
    ...
    userObject.assignedScreens = assignedScreensToSave;
